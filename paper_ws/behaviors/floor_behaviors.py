...
# def find_nearest_part(part_list, base_x, base_y):
#     if not part_list:
#         return None

#     def calculate_horizontal_distance(part):
#         return math.sqrt((part.pose.position.x - base_x)**2 + (part.pose.position.y - base_y)**2)

#     nearest_part = min(part_list, key=calculate_horizontal_distance)
#     return nearest_part
...
class GRASP(py_trees.behaviour.Behaviour):
    ...
    def initialise(self):
        print("开始发送抓取消息")
        self.grasp_part_action=ActionClient(self.node, Grasp, 'grasp_1')
...
class PLACE(py_trees.behaviour.Behaviour):
    ...
    def initialise(self):
        self.place_part_action=ActionClient(self.node, Grasp, 'grasp_1')
...
class MOVE(py_trees.behaviour.Behaviour):
    ...
    def initialise(self):
        self.move_action=ActionClient(self.node, Grasp, 'grasp_1')
...
class FLIP(py_trees.behaviour.Behaviour):

    def __init__(self, name,part_type,part_pose, node ):
        super(FLIP, self).__init__(name)
        self.node = node
        self.part_type=part_type
        self.part_pose=part_pose                              # 0抓  1放 2移动 3翻转 
        self.goal_status=None
    ...
    def initialise(self):
        self.flip_part_action=ActionClient(self.node, Grasp, 'grasp_1')
...
class Robot_Normal(py_trees.behaviour.Behaviour):

    def __init__(self, name,node):
        super(Robot_Normal, self).__init__(name)
        self.node = node
...
class Gripper_State(py_trees.behaviour.Behaviour):

    def __init__(self, name,node):
        super(Gripper_State, self).__init__(name)
        self.node = node
        self.floor_robot_gripper_state=None

    def initialise(self):
        self.floor_robot_gripper_state_sub = self.node.create_subscription(VacuumGripperState, 
                    '/ariac/floor_robot_gripper_state', self.floor_robot_gripper_state_cb, qos_profile_sensor_data)
    def update(self):
        
        if self.floor_robot_gripper_state:
    ...
    def floor_robot_gripper_state_cb(self, msg: VacuumGripperState):
        ...
        print("夹爪的状态",self.floor_robot_gripper_state)


class C_MOVE(py_trees.behaviour.Behaviour):
    def __init__(self, name,part_type,part_pose,dest, node ):
        super(C_MOVE, self).__init__(name)
        self.node = node
        self.part_type_1=part_type
        self.part_pose_1=part_pose                             # 0抓  1放 2移动 3翻转 
        self.dest_1=dest
